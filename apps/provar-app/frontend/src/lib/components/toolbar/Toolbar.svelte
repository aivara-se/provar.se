...
{#if editorStore.selectedFilePath}
  <div
    class="pointer-events-none absolute top-[8px] left-1/2 z-50 flex -translate-x-1/2 items-center gap-2 select-none"
  >
    <div
      bind:this={toolbarEl}
      class="pointer-events-auto flex h-[26px] items-center rounded-full border border-zinc-800/80 bg-[#161b22]/80 shadow-sm backdrop-blur-sm divide-x divide-zinc-800/80 text-xs"
    >
      <div
    ...
    {#if !editorStore.diagnostics.isValid || editorStore.diagnostics.warnings.length > 0}
      ...
      <button
        ...
        class="pointer-events-auto flex h-[26px] cursor-pointer items-center gap-1.5 rounded-full border border-amber-500/30 bg-[#161b22]/80 px-2.5 text-xs font-medium text-amber-400 shadow-sm backdrop-blur-sm transition-colors hover:border-amber-500/50 hover:bg-[#1c2128] hover:text-amber-300 focus:outline-none"

...
<Card shadow={false} padding="none" class="flex flex-1 py-2 px-3">
	<P class="text-sm font-semibold text-gray-700">Net Promoter Score</P>
	{#if values.length === 0}
		<P class="text-sm text-gray-500">No data</P>
	{:else}
		<main class="flex h-24 gap-2 py-2">
			<div class="flex py-2">
				<ValueDonutChart {value} color="#EC407A" />
			</div>
			<div class="flex flex-1 border-b border-l border-dashed border-gray-300">
				<MinimalLineChart min={0} max={100} stepX={50} {points} label="NPS" color="#EC407A" />
			</div>
		</main>
	{/if}
</Card>
